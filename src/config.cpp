 ...
 * Does not destroy the string in the process.
 */
UINT GetHotkeyChar( const char * const hotkeyString )
{
	char keyString[64];
	UINT result = 0;
	char *p;
	...
	// backup the string so that we don't destroy the original
	strcpy( keyString, hotkeyString );
...
BOOL RegisterSingleHotkey( ATOM atom, char* regKey, char *errorMsg, char *defaultKey )
{
	...
	if( ! modifiers )
	{
		...
		sprintf( msg, "Hotkey (%s) has no modifiers selected, skipping", hotkeyString );
		MessageBox( hwndMain, msg, "Win32WM Error", MB_OK | MB_ICONEXCLAMATION );
	}
	else if( !RegisterHotKey( hwndMain, atom, modifiers, hotkeyChar ) )
	{
		char msg[128];

		sprintf( msg, "Unable to register hotkey (%s) for %s", hotkeyString, errorMsg );
		MessageBox( hwndMain, msg, "Win32WM Error", MB_OK | MB_ICONEXCLAMATION );
...
BOOL RegisterAllHotkeys()
{
	struct _HOTKEYCONST
	{ 
		BOOL useHotKey;		// whether hotkey is active
	...
	{
		{ useVertical,   atomVertical,   "MaximizeVertical",   "Maximize Vertically",   "WIN+V" },
		{ useHorizontal, atomHorizontal, "MaximizeHorizontal", "Maximize Horizontally", "WIN+H" },
		{ useMaximize,   atomMaximize,   "Maximize",           "Maximize",			    "WIN+X" },
		{ useMinimize,   atomMinimize,   "Minimize",           "Minimize",              "WIN+Z" },
		{ useBackground, atomBackground, "SendToBackground",   "Send To background",    "WIN+B" }
	};
	...
	{
		if(hotkeyConstants[i].useHotKey)
